...
public class user_database extends SQLiteOpenHelper {
    ...
    public user_database(Context context) {
        //constructor ho
        //we gotta use super method always
        //    public SQLiteOpenHelper(Context context, String name, CursorFactory factory, int version) {
        super(context, DATABASE_NAME, null, 1);

        //steps to run query on a db
        //instantiate a db class that extends a SQLiteOpenHelper
        //open the db
        //run query
        //close
        //but we are only creating here so dont even think about opening it before the creation lol nice joke
        SQLiteDatabase db = this.getWritableDatabase();
        db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_NAME +" (_id INTEGER PRIMARY KEY AUTOINCREMENT,salutation TEXT, fname TEXT, lname TEXT, nationality TEXT, phone VARCHAR,actype VARCHAR, license VARCHAR, vehicle VARCHAR, username VARCHAR, password VARCHAR, status VARCHAR)");
    ...
    public boolean insertUserData(String salute,String fname,String lname,String country,String phone,String actype,String license,String vehicle,String username, String password,String statuss) {
        SQLiteDatabase db = this.getWritableDatabase();
